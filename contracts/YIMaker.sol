...
contract YIMaker {
    ...
    constructor(IEZZCFactory _factory, address _bar, address _yi, address _weth) public {
        factory = _factory;
        yi = _yi;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, yi), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, yi), new bytes(0));
        return amountOut;
    }

    function _toYI(uint256 amountIn) internal {
        IEZZCPair pair = IEZZCPair(factory.getPair(weth, yi));
        (uint reserve0, uint reserve1,) = pair.getReserves();
        address token0 = pair.token0();
